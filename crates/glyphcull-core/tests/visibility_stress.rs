//! Stress and memory-regression tests for the visibility system: culling
//! over a 100k-chunk document is bounded, deterministic, and stays within the
...
fn culling_peak_memory_within_budget() {
    ...
    let budget = bytes.len() * 8;
    eprintln!(
        "visibility cull peak: {peak} bytes over 25 culls of a {} byte package (budget {budget})",
        bytes.len()
    );
    assert!(peak <= budget, "cull peak {peak} exceeds budget {budget}");
}
